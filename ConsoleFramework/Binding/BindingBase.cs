...
public class BindingBase
{
    ...
    public void UpdateTarget()
    {
        ...
        try
        {
            ...
            { // Work with usual property
                ...
                //
                if (adapter == null)
                    targetPropertyInfo.GetSetMethod().Invoke(target, [converted]);
                else
    ...
    /// </summary>
    public static void ApplyChanges(IList destList, IList srcList, Observables.ListChangedEventArgs args)
    {
        switch (args.Type)
        {
    ...
    public void UpdateSource()
    {
        if (realMode != BindingMode.OneWayToSource && realMode != BindingMode.TwoWay)
            throw new Exception($"Cannot update source in {realMode} binding mode.");
        ignoreSourceListener = true;
        try
        {
            object targetValue;
            ...
            { // Work with usual property
                ...
                if (null != converter)
                {
                    ...
                    if (!result.Success)
                    {
                        ...
                        if (updateSourceIfBindingFails)
                        {
                            ...
                            sourcePropertyInfo.GetSetMethod().Invoke(source, [null]);
                        }
                        return;
                    }
                ...
                sourcePropertyInfo.GetSetMethod().Invoke(source, [convertedValue]);
    ...
    public void Bind()
    {
        ...
        if (targetPropertyClass != sourcePropertyInfo.PropertyType)
        {
            ...
            if (needConverter)
            {
                ...
                if (converter == null)
                    throw new Exception($"Converter for {targetPropertyClass.Name} -> {sourcePropertyInfo.PropertyType.Name} classes not found.");
            }
        }
