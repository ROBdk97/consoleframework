...
public class ModifierKeysConverter : ITypeConverter
{
    ...
    {
        if (source is not string) throw new NotSupportedException("Cannot convert from this object.");
    ...
    private static ModifierKeys GetModifierKeys(string modifiersToken)
    {
        ...
        if (modifiersToken.Length != 0)
        {
            ...
            do
            {
                ...
                string str = (length < 0) ? modifiersToken : modifiersToken[..length];
                ...
                modifiersToken = modifiersToken[(length + 1)..];
            } while (length != -1);
        }
        return none;
    }
