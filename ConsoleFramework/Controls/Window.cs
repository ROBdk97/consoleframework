...
public class Window : Control
{
    ...
    protected static void RenderBorders(RenderingBuffer buffer, Point a, Point b, bool singleOrDouble, Attr attrs)
    {
        if (singleOrDouble)
        {
            // Corners
    ...
    {
        Attr borderAttrs = IsActiveWindow() ? ControlTheme.WindowBorderActive : ControlTheme.WindowBorderInactive;
        if (moving || resizing)
        {
            borderAttrs = ControlTheme.WindowBorderInteraction;
        }
        ...
        // Borders
        Point bottomRight = new(ActualWidth - 3, ActualHeight - 2);
        RenderBorders(buffer, new Point(0, 0), bottomRight, moving || resizing, borderAttrs);
        ...
        if (resizing)
        {
            buffer.SetPixel(bottomRight.X, bottomRight.Y, UnicodeTable.SingleFrameBottomRightCorner,
                ControlTheme.WindowBorderInteraction);
        }
        // close button
        if (ActualWidth > 4)
        {
            ...
            buffer.SetPixel(3, 0, showClosingGlyph ? UnicodeTable.WindowClosePressedSymbol : UnicodeTable.WindowCloseSymbol,
                ControlTheme.WindowCloseButton);
            buffer.SetPixel(4, 0, ']');
        }
        // title
        if (!string.IsNullOrEmpty(Title))
        {
            ...
            if (availablePixelsCount > 0)
            {
                ...
                {
                    renderTitleString = Title[..availablePixelsCount];
                    if (renderTitleString.Length > 2)
                    {
                        renderTitleString = string.Concat(renderTitleString.AsSpan(0, renderTitleString.Length - 2), "..");
                    }
                    else
                    {
                        renderTitle = false;
